...
@section('content')
    <div class="w-full px-6 py-6 mx-auto">
        ...
        <form action="{{ route('posyandu.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            @csrf
            ...
            <div>
                <label class="block text-sm font-medium">Upload Media (bisa lebih dari satu)</label>
                <input type="file" name="files[]" multiple class="w-full border px-3 py-2 rounded">
                <p class="text-xs text-gray-500 mt-1">Maks 5MB per file. JPG, PNG, MP4, PDF.</p>
            </div>
